...
class XuLy extends Provider{
	...
	function AddUser($fullname,$email,$password){
		$sql = "INSERT INTO users(fullname,email,password,avatar) VALUES (?,?,?,'default-avatar.jpg')";
	...
	function LoadAllUser($id){//Lấy ds đề cử bạn bè
	...
	function SelectRequestAddFriend( $user2Id){ //Lấy ra danh sách người gửi yêu cầu kết bạn 
		$sql = "SELECT a.* FROM friends a WHERE a.user2Id = ? AND NOT EXISTS(select b.* from friends b where b.user1Id = ? and b.user2Id = a.user1Id)  ";
		return $this->LoadMore($sql,[$user2Id,$user2Id]);
	}function SelectSendRequestAddFriend( $user2Id){ //Lấy ra danh sách người được gửi yêu cầu kết bạn
		$sql = "SELECT a.* FROM friends a WHERE a.user2Id = ? AND NOT EXISTS(select b.* from friends b where b.user2Id = ? and b.userId = a.user1Id)  ";
		return $this->LoadMore($sql,[$user2Id,$user2Id]);
	}
}
?>